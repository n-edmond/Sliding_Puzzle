...
public:
  ...
  //NODES
	...
	bool can_move(int x, int y, int pos);//checks if this slot can be used

  //GETTERS
	pair<int, int> get_loc_of_space()const;//gets loc of 0
	int get_f() const;//gets f
	void get_count();//number of moves
...
private:
	...
	int g = 0;//path length
	int f = 0;//g + h 
	int move_count = 0;//used to count how many moves were used. 
